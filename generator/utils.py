...
# --- HELPER 2: Text Replace (Fixed for Vertical Stacking Bug) ---
...
# --- HELPER 8: PPTX TO PDF CONVERTER ---
...
def generate_pptx(form_data, uploaded_images, uploaded_videos, template_path, output_path):
    ...
    user_heading_font = form_data.get("heading_font", "Arial")
    user_heading_size = form_data.get("heading_size", "") # Format: "40", "36"
    ...
    st = {
        "[about_main_title]": form_data.get("about_main_title", ""),
        "[problem_main_title]": form_data.get("problem_main_title", ""),
        "[solution_main_title]": form_data.get("solution_main_title", ""),
        ...
        "[who_we_are_21]": form_data.get("who_we_are_s21", ""),
        "[our_goal_21]": form_data.get("our_goals_s21", ""),
        "[who_we_are_pt1]": form_data.get("who_we_are_p1", ""),
        "[who_we_are_pt2]": form_data.get("who_we_are_p2", ""),
        "[our_goals_pt1]": form_data.get("our_goals_p1", ""),
        "[our_goals_pt2]": form_data.get("our_goals_p2", ""),
        "[clients_main_title]": form_data.get("clients_heading", ""),
        "[s23_main_title]": form_data.get("s23_title", ""),
        "[s23_lbl1]": form_data.get("s23_desc_1", ""),
        "[s23_lbl2]": form_data.get("s23_desc_2", ""),
        "[s23_lbl3]": form_data.get("s23_desc_3", ""),
        "[s23_lbl4]": form_data.get("s23_desc_4", ""),
        "[s24_main_title]": form_data.get("s24_title", ""),
        "[s24_h1]": form_data.get("s24_sub_1", ""),
        "[s24_h2]": form_data.get("s24_sub_2", ""),
        "[s24_h3]": form_data.get("s24_sub_3", ""),
        "[s24_c1_p1]": form_data.get("s24_c1_b1", ""),
        "[s24_c1_p2]": form_data.get("s24_c1_b2", ""),
        "[s24_c2_p1]": form_data.get("s24_c2_b1", ""),
        "[s24_c2_p2]": form_data.get("s24_c2_b2", ""),
        "[s24_c3_p1]": form_data.get("s24_c3_b1", ""),
        "[s24_c3_p2]": form_data.get("s24_c3_b2", ""),
        "[s25_main_title]": form_data.get("s25_title", ""),
        "[s25_h1]": form_data.get("s25_sub_1", ""),
        "[s25_h2]": form_data.get("s25_sub_2", ""),
        "[s25_h3]": form_data.get("s25_sub_3", ""),
        "[s25_h4]": form_data.get("s25_sub_4", ""),
        "[s26_main_title]": form_data.get("s26_title", ""),
        "[s26_h1]": form_data.get("s26_sub_1", ""),
        "[s26_h2]": form_data.get("s26_sub_2", ""),
    }
    ...
    b_pts = {
        ...
        "[s25_c1_pts]": [p.strip() for p in form_data.get("s25_c1_text", "").split('\n') if p.strip()],
        "[s25_c2_pts]": [p.strip() for p in form_data.get("s25_c2_text", "").split('\n') if p.strip()],
        "[s25_c3_pts]": [p.strip() for p in form_data.get("s25_c3_text", "").split('\n') if p.strip()],
        "[s25_c4_pts]": [p.strip() for p in form_data.get("s25_c4_text", "").split('\n') if p.strip()],
        "[s26_c1_pts]": [p.strip() for p in form_data.get("s26_c1_text", "").split('\n') if p.strip()],
        "[s26_c2_pts]": [p.strip() for p in form_data.get("s26_c2_text", "").split('\n') if p.strip()],
        "[s27_main_title]": form_data.get("s27_title_val", ""),
        "[s27_sub_heading]": form_data.get("s27_sub_val", ""),
        "[s26_psh1]": form_data.get("s26_psh1_val", ""),
        "[s26_ds1]": form_data.get("s26_ds1_val", ""),
        "[s26_psh2]": form_data.get("s26_psh2_val", ""),
        "[s26_ds2]": form_data.get("s26_ds2_val", ""),
        "[s26_psh3]": form_data.get("s26_psh3_val", ""),
        "[s26_ds3]": form_data.get("s26_ds3_val", ""),
        "[s26_psh4]": form_data.get("s26_psh4_val", ""),
        "[s26_ds4]": form_data.get("s26_ds4_val", ""),
    }

    # 🔥 COMPLETE MASTER LIST OF HEADING PLACEHOLDERS 🔥
    main_heading_tags = [
        "[intro_main]", "[about_main_title]", "[problem_main_title]", 
        "[erp_main]", "[sol_main]", "[solution_main_title]", "[sol_used_main]", 
        "[pre_post_main]", "[ai_cap_main]", "[demo_main]", "[heat_map_main]", 
        "[cloud_main]", "[action_main]", "[work_main]", "[partner_main]", 
        "[proj_main]", "[us_main]", "[recog_main]", "[team_main]", 
        "[thank_main]", "[Main_heading_intro2]", "[clients_main_title]", 
        "[s23_main_title]", "[s24_main_title]", "[s25_main_title]", 
        "[s26_main_title]", "[s27_main_title]"
    ]
    ...
    for item in chosen_deck:
        ...
        if source_idx < len(prs_source.slides):
            ...
            shapes_to_process = list(cloned_slide.shapes)
            
            for shape in shapes_to_process:
                # 1. THEME COLOR
                apply_theme_color(shape, user_theme_color)

                # 2. CHECK IF THIS SHAPE IS A MAIN HEADING (Before tags are replaced)
                txt = get_all_text(shape)
                is_main_heading = any(h_tag in txt for h_tag in main_heading_tags)

                # 3. TEXT & BULLETS REPLACEMENT
                ...
                # 4. 🔥 FONT AND SIZE CHANGER (After text is replaced, but safely targeting the detected shape)
                if is_main_heading and shape.has_text_frame:
                    for paragraph in shape.text_frame.paragraphs:
                        for run in paragraph.runs:
                            # Apply Custom Font
                            if user_heading_font:
                                run.font.name = user_heading_font
                            # Apply Custom Size
                            if user_heading_size and str(user_heading_size).isdigit():
                                run.font.size = Pt(int(user_heading_size))

                # 5. IMAGES REPLACEMENT
                if hasattr(shape, "shape_type") and shape.shape_type == 13:
                    ...
                    img_tags = ["intro_img", "erp_img_1", "erp_img_2", "ai_cap_img_1", "ai_cap_img_2", 
                                ...
                                "team_img_1", "team_img_2", "team_img_3", "team_img_4", "thank_img", "intro_2_img"]
                    for t in range(1, 5): img_tags.append(f"s23_img_{t}")
                    for t in range(1, 5): img_tags.append(f"pre_post_img_{t}")
                    for t in range(1, 14): img_tags.append(f"client_logo_{t}")
                    for t in range(1, 4): img_tags.append(f"s27_img_{t}") 
                    
                    for t in img_tags:
                        if t in alt:
                            if t.startswith("client_logo_"):
                                logo_num = t.split("_")[-1]
                                if form_data.get(f"remove_logo_{logo_num}") == "yes":
                                    try:
                                        sp = shape._element
